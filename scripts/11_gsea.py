...
Two gene lists are tested:
  • upregulated paralogs: paralog_gene where mean Δz > 0 across tested cell lines
  • top-upregulated paralogs: paralog_gene in top quartile by mean Δz

Gene sets queried: KEGG_2021_Human, GO_Biological_Process_2023, Reactome_2022

Outputs (saved to results/gsea/)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  upregulated_paralogs.txt      — gene list submitted
  enrichr_<gene_set>.csv        — enrichment results per database
  figures/gsea/01_enrichment_dotplot.pdf

Usage:
    python scripts/11_gsea.py
    python scripts/11_gsea.py --min-dz 0.5   # only include paralogs with mean Δz ≥ 0.5
"""
...
# ── Figure ────────────────────────────────────────────────────────────────────
def plot_dotplot(all_results: pd.DataFrame) -> None:
    ...
    ax.set_ylabel("")
    ax.set_title("Enrichr pathway enrichment\nUpregulated paralog genes", fontsize=11)
...
def main() -> None:
    ...
    parser.add_argument("--min-dz", type=float, default=0.0,
                        help="Minimum mean Δz to include a paralog gene (default: 0.0)")
    ...
    if len(gene_list) < 3:
        print(f"[warn] Only {len(gene_list)} genes — enrichment unlikely to be informative. "
              "Try --min-dz 0.0 to include more genes.")

    # Save gene list
    gene_list_path = OUT_DIR / "upregulated_paralogs.txt"
    ...
    try:
        user_list_id = enrichr_submit(gene_list, description="upregulated_paralogs")
        print(f"  Enrichr list ID: {user_list_id}")
    except Exception as e:
        print(f"  ERROR: Could not connect to Enrichr: {e}")
        print("  Check your internet connection or try the web interface at:")
        print("  https://maayanlab.cloud/Enrichr")
        return
