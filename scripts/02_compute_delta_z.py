...
CCLE_REF  = DATA_DIR / "ccle_expression_paralogs.csv"

# Map cell-line → CCLE expression column (generated by 13_ccle_expression_filter.py)
CELL_LINE_CCLE_COL = {
    "K562":           "K562_log2tpm",
    "K562_essential": "K562_log2tpm",
    "rpe1":           "rpe1_log2tpm",
}
...
CELL_LINE_FILES = {
    ...
    "neuron":         "neuron_pseudobulk_normalized.h5ad",           # generated by 08_preprocess_neuron.py
    "iPSC":           "iPSC_KOLF2_pseudobulk_normalized.h5ad",       # generated by 17_preprocess_ipsc.py
    "iPSC_cc":        "iPSC_KOLF2_pseudobulk_cc_corrected.h5ad",    # cell-cycle corrected
}
...
def run(cell_line: str) -> None:
    ...
    # ── CCLE expression reference (for expression-level filtering) ─────────────
    ccle_lookup: dict[str, float] = {}
    ccle_col = CELL_LINE_CCLE_COL.get(cell_line)
    if ccle_col and CCLE_REF.exists():
        ccle_df = pd.read_csv(CCLE_REF).set_index("gene")
        if ccle_col in ccle_df.columns:
            ccle_lookup = ccle_df[ccle_col].dropna().to_dict()
            print(f"  CCLE expression loaded: {len(ccle_lookup)} genes for {cell_line}")
    elif ccle_col:
        print(f"  [info] CCLE reference not found — run 13_ccle_expression_filter.py to add paralog_ccle_log2tpm.")
    ...
    for _, row in tqdm(sig.iterrows(), total=len(sig), desc="sig pairs"):
        ...
        sig_rows.append({
            ...
            "paralog_ccle_log2tpm":  ccle_lookup.get(row["paralog_gene"], np.nan),
        })
    ...
    for _, row in tqdm(nonsig.iterrows(), total=len(nonsig), desc="non-sig pairs"):
        ...
        if dz1 is not None:
            ...
            nonsig_rows.append({
                ...
                "paralog_ccle_log2tpm": ccle_lookup.get(g2, np.nan),
            })
        ...
        if dz2 is not None:
            ...
            nonsig_rows.append({
                ...
                "paralog_ccle_log2tpm": ccle_lookup.get(g1, np.nan),
            })
